...
{//Start: Madison
 ...
 cout << "Value of atom 1 to distinguish in QM: " << QMMMData[1].QMRegion << '\n';
 cout.flush();
 if(SCFPol == 1)
 {
   ...
   for (int i=0;i<Natoms;i++)
   {
    cout << "QMMMData[i].QMRegion= " << QMMMData[i].QMRegion << '\n';
    cout << "QMMMData[i].P[bead].x= " << QMMMData[i].P[bead].x << '\n';
    cout.flush();
//    if (QMMMData[i].QMRegion or QMMMData[i].PBRegion)
    if (!QMMMData[i].MMRegion)
    {
       totM += QMMMData[i].m;
       cout << "Value of totM: " << totM << '\n';
       cout.flush();
     }
   }
   for (int i=0;i<Natoms;i++)
   ...
   cout << "Center of Mass for QM: " << COMqm.x << '\n';
   cout << "Center of Mass for QM: " << COMqm.y << '\n';
   cout << "Center of Mass for QM: " << COMqm.z << '\n';
   cout.flush();
